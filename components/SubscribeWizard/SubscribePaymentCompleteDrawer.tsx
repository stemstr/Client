    ...
    <SubscribeDrawer opened={opened} onClose={onClose} {...rest}>
      ...
      <Group mt={32} position="apart">
        <Box>
          ...
          {isPaymentComplete && (
            <Text mt={40} fz="sm">
              You're good to go! Once your time is up, you can pay again to
              regain access to sharing and commenting on Stemstr.
            </Text>
          )}
        </Box>
        {isDesktop && invoice && !isPaymentComplete && (
          <Center h={160} w={160} bg="white" sx={{ borderRadius: 10 }}>
            <QRCode size={144} value={invoice} />
          </Center>
        )}
      </Group>
      {isPaymentComplete ? (
